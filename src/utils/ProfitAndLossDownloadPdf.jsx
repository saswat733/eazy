...
const ProfitAndLossDownloadPDF = ({ data, companyName, date, dateFormat, size, username }) => {
  ...
  function generateProfitAndLossPDF() {
    ...
    doc.setFontSize(16);
    addText("Profit & Loss Statement", pageWidth / 2, { align: "center" });
    ...
    const renderSection = (section) => {
      ...
      section.subsections.forEach(subSection => {
        // Sub-section Header with extra spacing
        ...
        subSection.categories.forEach(category => {
          ...
          // Category Value formatted using formatNumberToUS
          const formattedValue = formatNumberToUS(category.value);
          doc.text(formattedValue, pageWidth - rightMargin, currentY, { align: "right" });
          
          currentY += styles.lineHeight;
        });

        // Sub-section Total formatted using formatNumberToUS
        checkPageBreak(styles.lineHeight);
        doc.setFontSize(10);
        doc.setTextColor(0, 0, 0);
        doc.text(`Total ${subSection.name}`, leftMargin + 5, currentY);
        doc.text(formatNumberToUS(subSection.total), pageWidth - rightMargin, currentY, { align: "right" });
        drawLine(currentY + 2);
        currentY += 10;
      });

      // Main Section Total formatted using formatNumberToUS
      checkPageBreak(styles.lineHeight);
      doc.setFontSize(12);
      doc.setTextColor(...styles.mainSectionHeader.color);
      doc.text(`Total ${section.name}`, leftMargin, currentY);
      doc.text(formatNumberToUS(section.total), pageWidth - rightMargin, currentY, { align: "right" });
      drawLine(currentY + 2);
      currentY += 12;
    };
  ...
  return (
    <Button sx={{ backgroundColor: '#d3d3d3', width: '200px' }} size={size} onClick={generateProfitAndLossPDF}>
      <PictureAsPdf sx={{ marginRight: "10px", padding: "2px 5px" }} /> Download PDF
    </Button>
  );
};

export default ProfitAndLossDownloadPDF;
